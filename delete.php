				...
				<?php
					...
					if ( isset($_POST["nombre"])){
						...
						if ( $enlace= conectarBase($host,$usuario,$clave,$base) ){
							$nombre = $_POST["nombre"];
							$consulta = "DELETE FROM jugador WHERE nombre='$nombre'";
							
							if ( mysqli_query($enlace,$consulta) && mysqli_affected_rows($enlace)>0 ){
								echo "<p>Registro eliminado.</p>";
								echo "<form method='post' action='formularioBorrar.php'><input type='submit' value='Volver Formulario'></form>";
							} else {
