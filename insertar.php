		...
		<section>
			<article id="cambiar">
				<?php
					...
					if ( $enlace=conectarBase($host,$usuario,$clave,$base) ){
						...
						if (isset($_POST["nombre"],$_POST["equipo"],$_POST["posicion"],$_POST["pais"])){
							...
							$consulta = "INSERT INTO jugador (nombre,equipo,posicion,pais) VALUES ('$nombre','$equipo','$posicion','$pais')";
							
							if ( mysqli_query($enlace,$consulta) /* and mysqli_affected_rows($enlace)>0 */) {
								echo mysqli_affected_rows($enlace);
								echo "<p>Registro agregado.</p>";
								echo "<form method='post' action='formulario.php'><input type='submit' value='Volver Formulario'></form>";
							}else{
